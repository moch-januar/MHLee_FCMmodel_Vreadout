...
for j = 1:Nref
    ...
    Vo_j    = fcm_simulate_states(cfg, t, cfg.C, Cref_j, cfg.VR, cfg.t0, shape, dec_lin, tauRC_j, ufun);
...
%% Figure 1: Time-domain examples for three Cref values
 ...
 % Avoid duplicates by nudging the custom slot if it collides
   ...
   figure('Color','w','Position',[60 410 800 320]);
   tiledlayout(1, numel(idx_examples)-1, 'TileSpacing','compact','Padding','compact');
   ...
   for ii = 2:numel(idx_examples)
       ...
       nexttile; hold on; box on
           plot(t*1e3, VR_t, 'k:','LineWidth',2, 'DisplayName','V_R');
           for k = 1:Nstate
               plot(t*1e3, Vo_j(k,:), 'LineWidth', 2.0, 'Color', co(k,:), ...
                   'DisplayName', sprintf('V_o (%s)', cfg.labels{k}));
           end
         % read markers
           vread_j = interp1(t, Vo_j.', t_read, 'linear','extrap').';
           plot(t_read*1e3, vread_j, 'o', 'MarkerFaceColor','b', 'MarkerEdgeColor','b', ...
               'MarkerSize', 6, 'HandleVisibility','off');
           title(sprintf('@C_{ref} = %.0f pF', Cref_j*1e12));
           xlabel('Time [ms]','FontSize',14.5,'FontWeight','bold');
           if ii==2; ylabel('Voltage [V]','FontSize',14.5,'FontWeight','bold');end
           set(gca, 'FontSize',13,'FontWeight','bold','TickDir','in');
           grid on; xlim([0, max(t)*1e3]); ylim([0.001, 3.5])
...
%% Figure 2: Readout vs Cref + Level margin
 ...
 % Left: Vread vs Cref (per state)
   ...
   for k = 1:Nstate
       semilogx(Cref_vec*1e12, Vread_mat(k,:), 'LineWidth', 2.2, 'Color', co(k,:), ...
                'DisplayName', sprintf('state %s', cfg.labels{k}));
   end
   xlabel('C_{ref} [pF]','FontSize',14.5,'FontWeight','bold');
   ylabel(sprintf('V_o at t=%.2f ms [V]', t_read*1e3),'FontSize',14.5,'FontWeight','bold');
   set(gca, 'FontSize',13,'FontWeight','bold','TickDir','in');
